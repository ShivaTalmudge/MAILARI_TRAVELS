...
import { 
  LayoutDashboard, CalendarRange, User, LogOut, Menu, X, PlaneTakeoff, PlusCircle
} from 'lucide-react';
...
export default function CustomerLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 transition-transform duration-300 md:static md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 items-center gap-2 border-b border-slate-100 px-6 font-bold text-brand-900">
          ...
          <button className="ml-auto text-slate-400 md:hidden" onClick={() => setSidebarOpen(false)}>
            <X size={20} />
          </button>
        </div>
        
        <div className="p-4">
          <Link to="/customer/book" onClick={() => setSidebarOpen(false)}>
